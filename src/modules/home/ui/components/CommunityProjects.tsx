...
export const CommunityProjects = () => {
  const trpc = useTRPC();
  const { data: screenshots, isLoading, error } = useQuery(trpc.projects.getLatestScreenshotsWithFragments.queryOptions());
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {screenshots.map((screenshot) => (
          <div key={screenshot.id} className="flex flex-col border rounded-lg overflow-hidden bg-card shadow-sm">
            <div className="p-3 border-b font-medium truncate">
              {screenshot.fragment?.title || "Project Preview"}
            </div>
            <div
            ...
            >
              <img
                ...
                alt={screenshot.fragment?.title || "Project Preview"}
